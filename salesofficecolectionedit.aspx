<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage.master" AutoEventWireup="true" CodeFile="salesofficecolectionedit.aspx.cs" Inherits="salesofficecolectionedit" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="Server">
    ...
    <script type="text/javascript">
        ...
        function getcollectiontypes() {
            var data = { 'operation': 'getcollectiontypes' };
            var s = function (msg) {
                if (msg) {
        ...
        function Bindcollectiontype(msg) {
            var ddlsalesOffice = document.getElementById('ddlcollection');
            ...
            for (var i = 0; i < msg.length; i++) {
                if (msg[i].type != null) {
                    var opt = document.createElement('option');
                    opt.innerHTML = msg[i].type;
                    opt.value = msg[i].type;
                    ddlsalesOffice.appendChild(opt);
                }
            }
        }
        ...
        function GetEditIndentValues() {
            ...
            var collectiontype = document.getElementById('ddlcollection').value;
            var data = { 'operation': 'GetEditCollectionamountClick', 'RouteID': ddlRouteName, 'IndDate': txtDate, 'collectiontype':collectiontype };
            var s = function (msg) {
                if (msg) {
                    $('#divFillScreen').removeTemplate();
        ...
        function btnEditCollectionSaveClick(id) {
            ...
            $(rows).each(function (i, obj) {
                if ($(this).find('#txtDeliveryQty').val() != "" || $(this).find('#txtDeliveryQty').val() != "0") {
                    ...
                    if (tot_qty > 0) {
                        Indentdetails.push({ Sno: $(this).find('#txtsNo').text(), BranchID: $(this).find('#txtIndentNo').text(), TripId: $(this).find('#txtunitQty').text(), PaidDate: $(this).find('#txtPaiddate').text(), PaidAmount: $(this).find('#txtDeliveryQty').val() });
                    }
                }
            });
            ...
            var data = { 'operation': 'btnEditsalesofficeCollectionSaveClick', 'data': Indentdetails, 'refno': ddlRouteName, 'indentdate': txtDate, 'Denominations': DenominationString, 'ColAmount': ColAmount, 'SubAmount': SubAmount, 'SalesOfficeID': soid };
            var s = function (msg) {
                if (msg) {
                    alert(msg);
                    $('#divFillScreen').removeTemplate();
                ...
                <table align="center">
                    ...
                    <tr>
                      ...
                      <tr>
                        ...
                        <td style="height:40px;">
                            <select id="ddlcollection" class="form-control">
                            </select>
                        </td>
                    </tr>
                    <tr>
