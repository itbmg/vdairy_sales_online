<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage.master" AutoEventWireup="true"
    CodeFile="StockEdit.aspx.cs" Inherits="StockEdit" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="Server">
    ...
    <script type="text/javascript">
        ...
        function ddlSalesOfficeChange(ID) {
            //FillProducts();
            var BranchID = ID;
            var data = { 'operation': 'GetBranchStock', 'BranchID': BranchID };
            var s = function (msg) {
                if (msg) {
                    //  BindProducts(msg);
        ...
        function getbranchinventory(ID) {
            var BranchID = ID;
            var data = { 'operation': 'GetBranchInventoryStock', 'BranchID': BranchID };
            var s = function (msg) {
                if (msg) {
                    //  BindProducts(msg);
        ...
        function btnPlantTripsaveclick() {
            ...
            $(rows).each(function (i, obj) {
                if ($(this).find('#txtProductQty').val() != "") {
                    Indentdetails.push({ Productsno: $(this).find('#hdnProductSno').val(), LeakQty: $(this).find('#txtleak').val(), StockQty: $(this).find('#txtStockQty').val() });
                }
            });
            ...
            var data = { 'op': 'btnEditBranchStock', 'data': Indentdetails, 'invdata': inventorydetails, 'refno': ddlsalesOffice };
            var s = function (msg) {
                if (msg) {
                    alert(msg);
                    $('#divFillScreen').removeTemplate();
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    <section class="content-header">
        <h1>
            Stock Clear<small>Preview</small>
        </h1>
        <ol class="breadcrumb">
            <li><a href="#"><i class="fa fa-dashboard"></i>Operations</a></li>
            <li><a href="#">Stock Clear</a></li>
        </ol>
    </section>
