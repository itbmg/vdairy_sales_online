<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage.master" AutoEventWireup="true" CodeFile="SalesOfficeCollectionsEdit.aspx.cs" Inherits="SalesOfficeCollectionsEdit" %>
    ...
    <script type="text/javascript">
        ...
        function ddlPaymentTypeChange(Payment) {
            ...
            if (PaymentType == "Cheque") {
                ...
                //                $('#divlabeldate').css('display', 'block'); 
        ...
        function get_EditCollections_details() {
            var fromdate = document.getElementById('txtFrmdate').value;
            var todate = document.getElementById('txtToDate').value;
            var data = { 'operation': 'get_EditCollections_details', 'fromdate': fromdate, 'todate': todate };
            var s = function (msg) {
                if (msg) {
                    fill_editcollectiondetails(msg);
                }
                else {
                }
            };
        ...
        function fill_editcollectiondetails(msg) {
            var results = '<div  style="overflow:auto;"><table id="tbl_collections" class="table table-bordered table-hover dataTable no-footer" role="grid" aria-describedby="example2_info">';
            results += '<thead><tr><th scope="col"></th><th scope="col">ReceiptNo</th><th scope="col">Name</th><th scope="col">Amount</th><th scope="col">CollectionType</th><th scope="col">CollectionFrom</th><th scope="col">Remarks</th></tr></thead></tbody>';
            var k = 1;
            var l = 0;
            ...
            for (var i = 0; i < msg.length; i++) {
                ...
                results += '<th scope="row"  class="1" style="text-align:center;">' + msg[i].Receiptno + '</th>';
                results += '<td     class="2">' + msg[i].Name + '</td>';
                results += '<td     class="3">' + msg[i].Amount + '</td>';
                results += '<td   class="4">' + msg[i].CollectionType + '</td>';
                results += '<td   class="5">' + msg[i].CollectionFrom + '</td>';
                results += '<td   class="6">' + msg[i].Remarks + '</td>';
                results += '<td  style="display : none;" class="7">' + msg[i].PaymentType + '</td>';
                results += '<td  style="display : none;" class="8">' + msg[i].Agentid + '</td>';
                results += '<td  style="display : none;" class="9">' + msg[i].DOE + '</td>';
                results += '<td  style="display : none;" class="10">' + msg[i].VEmpID + '</td>';
                results += '<td   style="display : none;" class="11">' + msg[i].CheckStatus + '</td>';
                results += '<td   style="display : none;" class="12">' + msg[i].ChequeNo + '</td>';
                results += '<td   style="display : none;" class="13">' + msg[i].VarifyDate + '</td>';
                results += '<td   style="display : none;" class="14">' + msg[i].ChequeDate + '</td>';
                results += '<td   style="display : none;" class="15">' + msg[i].BankName + '</td>';
                results += '<td   style="display : none;" class="16">' + msg[i].freezertype + '</td>';
                results += '<td   style="display : none;" class="17">' + msg[i].freezeramounttype + '</td>';
                results += '<td   style="display : none;" class="18">' + msg[i].cashcollectionsno + '</td>'
                results += '<td   style="display : none;" class="19">' + msg[i].TransType + '</td>'
                results += '<td   style="display : none;" class="20">' + msg[i].Branchid + '</td>'
                results += '<td   style="display : none;" class="21">' + msg[i].receiptsno + '</td>'
                results += '<td   style="display : none;" class="22">' + msg[i].ReceivedFrom + '</td>'
                results += '<td   style="display : none;" class="23">' + msg[i].reciptAmountPaid + '</td>'
                results += '<td   style="display : none;" class="24">' + msg[i].DOE + '</td>'
                results += '<td   style="display : none;" class="25">' + msg[i].Create_by + '</td>'
                results += '<td   style="display : none;" class="26">' + msg[i].cashReceipt + '</td>'
                results += '<td   style="display : none;" class="27">' + msg[i].PaymentStatus + '</td> </tr>';
            ...
            results += '</table></div>';
            $("#grid_cashcollections").html(results);
        }

        function getme(thisid) {
            $('#cashcollections_table').css('display', 'block');
            $('#showlogs').css('display', 'none');
            $('#grid_cashcollections').hide();
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    ...
    <section class="content">
        <div class="box box-info">
            ...
            <div class="box-body">
            <table id="divgrid" class="gridclass">
                  <tr>
                                    <td>
                                        <span>FromDate</span>
                                    </td>
                                    <td style="height: 40px;">
                                          <input type="date" id="txtFrmdate" class="form-control" name="fromdate"/>
                                    </td>
                                     <td>
                                        <span>ToDate</span>
                                    </td>
                                    <td style="height: 40px;">
                                          <input type="date" id="txtToDate" class="form-control" name="todate"/>
                                    </td>
                                     <td>
                                    </td>
                                    <td style="height: 40px;">
                                            <input type="button" id="btn_Generate" value="Generate" class="btn btn-primary" onclick="get_Collections_details();" />
                                    </td>
                                     <td style="height: 40px;">
                                     </td>
                                    <td style="height: 40px;">
                                     <div id="showlogs" align="center" style="height: 40px;width: 500px;"">
                    <input id="btn_collectionsEdit" type="button" name="submit" value='Collections' class="btn btn-primary" />
                </div>
                                    
                                    </td>
                                </tr>
                </table>
                <div id="grid_collectionsedit">
                </div>
                <table style="width: 100%;display:none;" id="tblcollectionEdit">
                    <tr>
                        <td style="width: 50%;">
                            <table align="center">
                                <tr>
